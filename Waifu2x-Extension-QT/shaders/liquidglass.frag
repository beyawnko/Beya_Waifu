...
// Group related uniforms in a UBO
layout(std140, binding = 1) uniform LiquidGlassParams {
...
vec3 calcNormalSDF(vec2 p, vec2 boxSize, float radius) {
    vec2 e = vec2(1.0, 0.0) / params.resolution.y; // Small epsilon scaled by pixel size for consistent thickness
...
void main() {
    ...
    vec2 uv = (gl_FragCoord.xy - 0.5 * params.resolution.xy) / params.resolution.y;
    ...
    float boxHeight = params.resolution.y * 0.4; // Box is 40% of widget height
    float boxWidth = boxHeight * 1.6;     // Box width (e.g., 1.6:1 aspect)
    vec2 boxHalfSize = vec2(boxWidth / 2.0, boxHeight / 2.0) / params.resolution.y; // Scale to UV space
    float actualBorderRadius = params.borderRadius / params.resolution.y; // Scale radius to UV space

    // Center the box (or use params.mouse.xy / params.resolution.xy - 0.5 to position it with params.mouse)
    vec2 boxCenter = vec2(0.0, 0.0); // Centered
    // vec2 boxCenter = (params.mouse - 0.5 * params.resolution.xy) / params.resolution.y; // Mouse controlled
    ...
    vec3 refractedDir = refract(-viewDir, normal, 1.0 / params.ior);
    ...
    vec2 caOffsetVec = normalize(uv - boxCenter) * params.chromaticAberrationOffset; // Offset outwards
    ...
    float borderThickness = 2.0 / params.resolution.y; // 2 pixels thick border
